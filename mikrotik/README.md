...
"Маршрутизируемые" сервисы (без amazon/microsoft) — около 430 маршрутов
(включая GitHub, Apple и т.д.) благодаря агрегации соседних подсетей в
`scripts/fetch.py` (`ipaddress.collapse_addresses`, см. главный README).
Для hAP ac lite это более чем безопасный объём — RouterOS штатно работает
с многими тысячами статических маршрутов даже на слабом железе.
