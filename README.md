...
| `data/apple.txt` | Apple: FaceTime/iMessage + весь остальной Apple (см. ниже) | ASN 714, 6185 |
...
`apple.txt` (FaceTime/iMessage) — та же логика в другом виде: у Apple нет
отдельного диапазона именно под FaceTime/iMessage, весь трафик компании идёт
через общий ASN (714 + 6185) вместе с App Store/iCloud/обновлениями и т.п.
В отличие от amazon/microsoft/cloudflare это не шаред-инфраструктура с чужими
клиентами — весь диапазон принадлежит одной компании, поэтому оставлен в
`routable_cidr_services` целиком: другого способа получить FaceTime/iMessage
по IP просто нет.

## Агрегация префиксов

`dedupe_and_sort()` в `scripts/fetch.py` не только убирает дубли, но и
схлопывает соседние/вложенные подсети в супернеты через
`ipaddress.collapse_addresses()`. Эффект заметный: Apple — 1971→11 записей,
Microsoft — 59396→4575, Amazon — 10684→3788, Meta — 757→118. Итоговый
`all.txt` вместо ~73 000 строк — около 8-9 тысяч. Это применяется ко всем
файлам одинаково, включая DNS-резолвленные (`discord.txt` и т.п.), хотя там
эффект обычно небольшой — отдельные `/32` редко оказываются соседними.
